...
//
///**
// * The infinite auto-expendable on 2^n lock-free list.<br>
...
//  public ExtendedList(final int size) {
...
//
//      // Add element
//      array[writeOperation.indexOfElement] = writeOperation.element;
//
//      // Complete write
...
//      completeWrite(currentDescriptor.writeOperation, 1);
//
...
//    completeWrite(operationDescriptor.writeOperation, 1);
//
//    return true;
//  }
//
...
//  }
//
//  private T getInner(final int index) {
//    return array[index];
//  }
//
//  /**
...
//      completeWrite(currentDescriptor.writeOperation, 1);
//
//      final int currenIndex = currentDescriptor.size - 1;
//      currentElement = getInner(currenIndex);
//
//      operationDescriptor = new Descriptor(currentDescriptor.size - 1, new WriteOperation<T>(currenIndex, currentElement, false));
//
//    } while (!descriptor.compareAndSet(currentDescriptor, operationDescriptor));
//
...
public class ExtendedList<T> extends AbstractList<T> implements List<T> {
   ...
   */
  private final List<AtomicBoolean>         rootArray;
  ...
  public ExtendedList() {
    ...
    rootArray = new ArrayList<AtomicBoolean>(ROOT_SIZE);
    for (int i = 0; i < ROOT_SIZE; i++)
      rootArray.add(new AtomicBoolean());
  ...
  protected void allocateBucket(final int bucket) {
    ...
    if (rootArray.get(bucket).compareAndSet(false, true))
  ...
  public T remove(final int index) {
    ...
    do {
      ...
      final int currenIndex = currentDescriptor.size - 1;
      currentElement = getUnchecked(currenIndex);
